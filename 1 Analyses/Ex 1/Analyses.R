...
#Now do t-tests
#IS vs RL
temp8 = t.test(Type.Combined$READ, Type.Combined$IS, paired = F, p.adjust.methods = "Bonferroni", var.equal = T)
...
RECALL3 = cast(RECALL1, Subject ~ Task, mean)
RECALL4 = cast(RECALL2[ , -8], Username ~ Task, mean)

temp1 = t.test(RECALL3$Recall, RECALL4$Recall, paired = F, var.equal = T)
SEM1 = (temp1$conf.int[2] - temp1$conf.int[1]) / 3.92
temp1
...
##Okay, these are overall gammas -- but there is a precedence in the literature to report mean gammas (Dunlosky and Nelson 1991 for example)
#So, loop time?!
